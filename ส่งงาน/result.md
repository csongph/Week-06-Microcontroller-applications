### Lab6_1
ขนาดของ Binary
<img width="567" height="331" alt="image" src="https://github.com/user-attachments/assets/39021e78-2488-4ee9-8132-9f939ff9797b" />
...
<img width="377" height="397" alt="image" src="https://github.com/user-attachments/assets/ce09a270-8436-41fc-ad5a-ac7500d00707" />

### คำถามทบทวน
1.Multiple Source Files: เหตุใดต้องแยก source code เป็นหลายไฟล์?

ตอบ เพื่อจัดระเบียบโค้ดให้เป็นหมวดหมู่ (เช่น โมดูลเซ็นเซอร์, LED, จอแสดงผล) ทำให้โค้ดอ่านง่าย เข้าใจง่าย และจัดการได้สะดวกขึ้น ลดเวลาในการคอมไพล์ เพราะเมื่อแก้ไขแค่ไฟล์เดียว ก็ไม่ต้องคอมไพล์โค้ดทั้งหมดใหม่

2.CMakeLists.txt Management: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?

ตอบ ต้องเพิ่มชื่อไฟล์ source ใหม่เข้าไปในตัวแปรที่เก็บรายชื่อไฟล์ เช่น set(SRCS "main.c" "new_module.c") หรือเพิ่มในคำสั่ง target_sources() เพื่อให้ CMake รู้จักและนำไปคอมไพล์

3.Header Files: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?

ตอบ ใช้ประกาศฟังก์ชัน, ตัวแปร, และโครงสร้างข้อมูล (structs) ที่ต้องการให้ไฟล์อื่นเรียกใช้งานได้ Header Files (.h) ทำหน้าที่เป็น interfaceที่บอกให้โค้ดส่วนอื่นรู้ว่าไฟล์นั้นๆ มีฟังก์ชันอะไรบ้างที่สามารถเรียกใช้งานได้ โดยไม่จำเป็นต้องรู้รายละเอียดการทำงานภายในทั้งหม

4.Include Directories: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?

ตอบ เพื่อบอก CMake และคอมไพล์เลอร์ว่าควรค้นหาไฟล์ Header (.h) ที่ถูก include มาจากที่ไหนบ้างหากไม่ระบุ คอมไพล์เลอร์จะไม่พบไฟล์ Header ที่อยู่ในโฟลเดอร์อื่น ทำให้เกิดข้อผิดพลาดในการคอมไพล์

5.Git Ignore: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?

ตอบ ช่วยป้องกันไม่ให้ไฟล์ที่ไม่จำเป็นต่อโปรเจกต์ถูกติดตามและอัปโหลดขึ้น Git repository ไฟล์เหล่านี้ได้แก่ ไฟล์ที่สร้างขึ้นโดยอัตโนมัติจากการคอมไพล์ (เช่น .bin, .elf), ไฟล์ล็อก, ไฟล์ตั้งค่าส่วนตัว, และโฟลเดอร์ build ทำให้ Git repository สะอาดและมีขนาดเล็ก

6.Task Management: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?

ตอบ ช่วยให้การทำงานของ LED (เช่น การกะพริบ) สามารถทำงานแบบอิสระและทำงานพร้อมกันกับส่วนอื่นๆ ของโปรเจกต์ได้ทำให้การทำงานของ LED ไม่ไปบล็อกการทำงานหลักของโปรเจกต์ เช่น การอ่านค่าเซ็นเซอร์

7.Code Organization: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?

ตอบ ข้อดีของการแยกโค้ดเป็นโมดูล (sensor, display, led) คือช่วยให้โค้ดเป็นระเบียบ แก้ไขได้ง่าย และ นำกลับไปใช้ใหม่ได้ ในโปรเจกต์อื่นครับ
...
<img width="737" height="56" alt="image" src="https://github.com/user-attachments/assets/6e75baa8-22ab-4b06-81d8-ed35b62e16ea" />

**ขั้นตอนที่ 2 (เพิ่ม display.c):**
- จำนวนไฟล์ source: 6
- ขนาด binary: 163925 bytes
- การทำงาน: ดึงค่า value มาใช้งาน
<img width="360" height="58" alt="image" src="https://github.com/user-attachments/assets/4377b680-f451-4c5d-8447-495b5c14effb" />

**ขั้นตอนที่ 3 (เพิ่ม led.c):**
- จำนวนไฟล์ source: 8
- ขนาด binary: 164873 bytes
- การทำงาน: แสดงผลข้อมูลทั้งหมด
<img width="552" height="496" alt="image" src="https://github.com/user-attachments/assets/86c2b488-b940-470b-9e76-115af1ae3c8d" />
